 ...
 * from this project's own copy of `docs/03_terms-and-conditions.md`
 * (specs/032-terms-conditions).
 *
 ...
 *   the internal team, not public visitors (FR-007).
 ...
 *   (FR-005; Assumptions) rather than rendered as literal brackets.
 ...
 *   (spec Edge Cases: rendering both would be two competing navigation lists
 *   on one page).
 *
 * The source document's two references to the Privacy Policy (§1, §18) point
 * at a stale relative path that doesn't match either project's actual
 * filename. Both are transcribed using PRIVACY_POLICY_HREF instead, so they
 * resolve to the live `/privacy` route (FR-009; research.md Decision 5).
 *
 * Editing the legal substance beyond those specific fallbacks/link fixes is
 * out of scope for this file (FR-012) — any future correction (including the
 * refund policy and dispute-venue wording the source document itself flags
 * as pending ACRET/legal sign-off) is ACRET/legal counsel's call, made by
 * editing docs/03_terms-and-conditions.md first and this transcription to
 ...
 *   PRIVACY_POLICY_HREF (research.md Decision 5).
 */
...
/** Resolved fallbacks for the source document's unresolved placeholders (FR-005). */
export const TERMS_META: TermsMeta = {
  effectiveDate: "August 19, 2026",
  lastUpdated: "August 19, 2026",
};

/** Live route for the Privacy Policy, replacing the source document's stale relative path (FR-009). */
export const PRIVACY_POLICY_HREF = "/privacy";

/** §20's real, already-written contact email (FR-006) — no fallback needed, unlike privacy-content.ts's contactEmail. */
export const CONTACT_EMAIL = "support@hapag.ph";

// Sourced from this project's own PLAN_PRICING (specs/014), not restated
// independently — matching src/lib/marketing/content.ts's PRICING_TIERS
// precedent (specs/030 research.md Decision 6) — so this page's figures
// cannot drift out of sync with the pricing actually charged.
export const PRICING_TABLE: PricingPlanEntry[] = [
  {
    plan: "Standard",
    monthlyFee: `₱${Number(PLAN_PRICING.standard)}/month`,
    included:
      "Digital menu with unlimited categories/items, business QR code, logo and photo uploads, and support tickets.",
  },
  {
    plan: "Pro",
    monthlyFee: `₱${Number(PLAN_PRICING.pro)}/month`,
    included:

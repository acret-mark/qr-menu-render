...
|---|---|---|
| Cold starts | Render's free Web Service spins down after 15 minutes with **no traffic** | **Confirmed live, 2026-09-14, two different ways with two different results.** An automated test (three `curl` samples against `/api/health` across a 20-minute window) never caught one — all returned in 0.17–0.29s. Working theory: `render.yaml`'s own `healthCheckPath` polling counts as traffic and kept resetting the idle timer during that test. But opening the URL in an actual browser after longer idle time *did* trigger a real cold start — Render's full branded "waking up" splash screen, confirmed directly (exact duration not timed). Net finding: cold starts are real and will be visible to an end user hitting this deployment after genuine idle time, even if they're hard to catch with synthetic polling that itself keeps the service warm. |

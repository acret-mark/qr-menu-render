...
**Status as of 2026-09-14: no company budget is currently approved for Render.** Per
`specs/003-render-feasibility-poc`'s 2026-09-11 reversal, this project runs on Render's free tier
— the `render-free-interim` branch's `render.yaml`, live at
https://qr-menu-render-interim.onrender.com since 2026-09-14 — as an **interim measure**, not the
target configuration (`main`'s `render.yaml`, which requires the paid Starter + Postgres Basic
tier — see `/Users/acretstaff/render/migration/render-migration-plan.md` for the cost comparison
and `specs/003-render-feasibility-poc`'s paid-tier request document for the evidence-backed ask).

**⚠️ Database expires 2026-10-14.** Recreate it before then per the steps below, or everything in
it is lost.
...
|---|---|---|
| Cold starts | Render's free Web Service spins down after 15 minutes with **no traffic** | **Measured 2026-09-14, not reproduced under normal operation**: three samples against `/api/health`, spaced minutes apart including a 20-minute gap with no deliberate traffic, all returned in well under 1 second (0.17–0.29s) — no cold-start penalty observed. Working theory: `render.yaml`'s own `healthCheckPath: /api/health` polls the service periodically, and that polling itself counts as traffic, resetting the 15-minute idle timer — so the health check may be inadvertently preventing the spin-down it exists to detect. Cold starts likely still happen on genuine first boot or if health checks themselves ever lapse; the <2s target (`docs/01_planning.md` in `qr-menu-dev`) was not violated in what was actually observed. Revisit if real inactivity (health check disabled or Render's polling interval exceeds 15 min) is ever confirmed to trigger one. |
| Database expiry | Render's free Postgres is **not permanent** — it is deleted after a limited period unless upgraded to a paid plan | **Confirmed live, 2026-09-14**: `qr-menu-render-interim-db` was created 2026-09-14 and Render's own dashboard states it **expires 2026-10-14** (exactly 30 days) — matches this migration's earlier research figure exactly. Recreation needed by that date; see the steps below. |
...
## What to do when the free Postgres instance expires (by 2026-10-14)
